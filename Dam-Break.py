...
yd = sn.Functional("yd", [xd, td], 4*[40], "tanh")
Vd = sn.Functional("Vd", [xd, td], 4*[40], "tanh")
...
alpha=10  # Causal decay rate
cumulative_residual = 0.0

# Spatial discretization (adding points near expected shocks)
x_vals = np.concatenate([
    np.linspace(-xd_max, -xd_max/2, 50),
    np.linspace(-xd_max/2, xd_max/2, 100),
    np.linspace(xd_max/2, xd_max, 50)
])
...
for interval in range(N_intervals):
    ...
    t_vals = np.linspace(td_start, td_end, 50)
